...
import type { EditorAuthedUser, EditorProjectFile, EditorProjectSummary } from '@/editor/types';

type SharedProjectViewProps = {
  shareSlug: string;
  project: EditorProjectSummary;
  files: EditorProjectFile[];
  viewer: EditorAuthedUser | null;
};
...
export default function SharedProjectView({ project, files, viewer }: SharedProjectViewProps) {
  ...
  async function handleRemix() {
    ...
    try {
      const created = await readJson<CreateProjectResponse>(
        ...
        <header className="flex flex-wrap items-center justify-between gap-4 rounded-[2rem] border border-white/8 bg-white/[0.03] p-5 shadow-[0_24px_90px_rgba(0,0,0,0.34)] backdrop-blur-[24px]">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-white/58">
              Read-only preview of the shared entry file. Remix it into your account to keep editing.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {viewer ? (
              ...
              <Link
                href="/login?message=Log%20in%20to%20remix%20this%20project.&kind=info"
